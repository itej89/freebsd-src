...
/* FreeBSD replacements for the two V4L2 types the shared structs referenced. */
struct wave5_pix_fmt {
	u32 width;
	u32 height;
};
...
/*
 * FreeBSD: the V4L2 device/m2m/video_device members are gone with the V4L2
 * layer they belonged to, as are the Linux clock, reset, hrtimer and kthread
 * handles -- wave5_freebsd.c owns those in its own softc, which this struct
 * is embedded in. Everything below is hardware state the ported files use.
 */
struct vpu_device {
	struct device *dev;
	struct list_head instances;
	struct mutex dev_lock; /* lock for the instance list */
...
struct vpu_instance {
	...
	/*
	 * FreeBSD: these were struct v4l2_pix_format_mplane. The ported
	 * hardware files read exactly one field between them
	 * (inst->src_fmt.width, in the encoder SRAM sizing path), so a plain
	 * geometry pair replaces the V4L2 types rather than dragging in a
	 * V4L2 header this kernel does not have.
	 */
	struct wave5_pix_fmt src_fmt;
	struct wave5_pix_fmt dst_fmt;
